...
function sanitize(doc: CandidateDocument): Omit<CandidateDocument, 'passwordHash'> {
...
export class CandidatesService {
  ...
  async updateMe(id: string, partial: Partial<CandidateDocument>) {
    const { email: _e, passwordHash: _pw, id: _id, createdAt: _ca, ...safe } = partial as Record<string, unknown>;
    void _e; void _pw; void _id; void _ca;
    await this.repo.update(id, { ...safe as Partial<CandidateDocument>, updatedAt: new Date().toISOString() });
  ...
  async uploadResume(id: string, buffer: Buffer, mimetype: string) {
    ...
    const { email: _e, id: _id, passwordHash: _pw, createdAt: _ca, ...profileFields } =
      extracted as Record<string, unknown>;
    void _e; void _id; void _pw; void _ca;
  ...
  async listAll(filters: { skills?: string; education?: string; experience?: string }) {
